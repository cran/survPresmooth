...
int *nkernel, int * dup, int *nestimand, double *phat, double *estim, int* presmoothing, double *isev){
			...
			for (boot = 0; boot < *nboot; boot++){
				...
				for (j = 0; j < *legridbw2; j++)
					for (i = 0; i < *legridbw1; i++){
						...
						simpson(integrand, legridise, isecomp);
						isev[boot * (*legridbw1) * (*legridbw2) + j * (*legridbw1) + i] = *isecomp;
					}
			}
		break;
// h
...
//			isev[boot * (*legridbw) + i] = (estimboot[0] - estim[0])*(estimboot[0] - estim[0])/2;
	//			for (j = 1; j < (*legridise - 1); j++)
	//				isev[boot * (*legridbw) + i] += (estimboot[j] - estim[j])*(estimboot[j] - estim[j]);
	//			isev[boot * (*legridbw) + i] += (estimboot[j] - estim[j])*(estimboot[j] - estim[j])/2;
						for (j = 0; j < *legridise; j++)
							integrand[j] = (estimboot[j] - estim[j])*(estimboot[j] - estim[j]);
						simpson(integrand, legridise, isecomp);
						isev[boot * (*legridbw2) + i] = *isecomp;
					}
				}
			}
			else{
