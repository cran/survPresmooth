control.presmooth <- 
function(n.boot = c(5000, 1000), q.weight = c(0.2, 0.8), k = 1, length.grid.bw.plugin = 100, length.grid.ise = 100, pilot.par.ini= NULL, save.data = FALSE, save.mise = FALSE, trim = 0, na.action = na.omit){
	list(n.boot = n.boot, q.weight = q.weight, k = k, length.grid.bw.plugin = length.grid.bw.plugin, length.grid.ise = length.grid.ise, pilot.par.ini = pilot.par.ini, save.data = save.data, save.mise = save.mise, trim = trim, na.action = na.action)
}
...
function(times, status, dataset = NULL, estimand = c("S", "H", "f", "h"), bw.selec = c("plug-in", "bootstrap", "fixed"), fixed.bw = NULL, grid.bw.pil = NULL, grid.bw = NULL, kernel = c("biweight", "triweight"), bound = c("none", "left", "right", "both"), x.est = NULL, control = control.presmooth()){ 
	...
	switch(n.bw.selec,
		{ # Plug-in
			...
			switch(estimand,
        	           ...
        	           bw <- control$k * range.t
                         ...
                         bw <- 0
		                    ...
		                    # Formula on page 91 of López-de-Ullibarri's thesis, modified by considering an empirical upper bound (trying to avoid the sporadical ocurrence of exceedingly large bandwidths)
		                  	pilot.bw1 <- min((C2/C1 * ifelse(C1 < 0, -1 , 3/2) / n)^(1/5), control$k * range.t)
							...
							D2 <- -const$c2 * .C("simpson", Dintegrand$d2int, le.plugin, step.plugin, integral = numeric(1), PACKAGE = "survPresmooth")$integral
		                  	...
		                  	pilot.bw2 <- min((D2/D1 * ifelse(D1 < 0, 1/2 , -1) / n)^(1/3), control$k * range.t)
                            ...
                            if(all(p.nw[cond] > 0.999) | all(p.nw[cond] < 0.001)){ # 3rd if-else
								...
								bw <- min((const$c5 * Q / A / 2 / const$c1^2 / n)^(1/3), control$k * range.t)
    	               		} # 3rd if-else
						} # 2nd if-else
					} # 1st if-else
				}, 
				f = {
                    ...
                    km <- .C("presmestim", dfr$t, n, dfr$t, n, NULL, NULL, NULL, as.numeric(dfr$d), NULL, as.integer(1), pest = numeric(n), PACKAGE = "survPresmooth")$pest
		...
		{# Bootstrap
            ...
            ps.dfr <- switch(estimand,
				S = ,
				H =	.C("presmestim", grid.ise, le.ise, dfr$t, n, NULL, NULL, NULL, p.hat, dup, n.estimand, pest = numeric(le.ise), PACKAGE = "survPresmooth")$pest,
			...
			# Compute bootstrapped presmoothed estimates
           	ise <- .C("isevect", dfr$t, dfr$d, n, n.boot, grid.ise, le.ise, grid.bw.1, le.bw.1, grid.bw.2, le.bw.2, n.kernel, dup, n.estimand, p.hat, ps.dfr, ise = numeric(if(forh) le.bw.1*le.bw.2*n.boot else le.bw.1*n.boot), PACKAGE = "survPresmooth")$ise
			ise.array <- if(forh) array(ise, dim = c(le.bw.1, le.bw.2, n.boot)) else array(ise, dim = c(le.bw.1, n.boot))
            mise <- apply(ise.array, if(forh) 1:2 else 1, function(x, trim) mean(x, trim = control$trim))
            ...
            pos.min.mise <- max(which(mise == min(mise)))
       	 	bw <- 
                  pmin(if(forh) c(grid.bw.1[(pos.min.mise - 1)%%le.bw.1 + 1], grid.bw.2[(pos.min.mise - 1)%/%le.bw.1 + 1])
      		           else grid.bw.1[pos.min.mise], control$k * range.t)
      	}, # End of bootstrap
		bw <- fixed.bw	# fixed 
    ...
    if(forh & (n.bound == 4) & (max(x.est) > bw[2]) & all((x.est < bw[2]) | (max(dfr$t) - x.est < bw[2])))
 	...
 	estim <- .C("presmestim", x.est, le.x.est, dfr$t, n, if(forh) bw[2], if(forh) n.kernel, if(forh) n.bound, p, dup, n.estimand, pest = numeric(le.x.est), PACKAGE = "survPresmooth")$pest
	ps <- list(call = match.call(), data = if(control$save.data) dfr, q.weight = q.w, bw.selec = bw.selec, mise = if((n.bw.selec == 2) & control$save.mise) if(forh) matrix(mise*step.ise, nrow = le.bw.1, ncol = le.bw.2) else mise*step.ise, trim = if (n.bw.selec == 2) control$trim, grid.pil = if(n.bw.selec != 3) grid.bw.pil, problem.pi = if(n.estimand <= 2 & n.bw.selec == 1) problem, pilot.bw = if(n.bw.selec != 3) pilot.bw, bandwidth = bw, grid.bw = if(n.bw.selec == 2) list(grid.bw.1 = grid.bw.1, grid.bw.2 = grid.bw.2), p.hat = p, x.est = x.est, estimand = estimand, estimate = estim)
...
function(x, more = NULL, ...){
    ...
    if(!is.null(more)){
      ext.names <- c(call = "Call", data = "Data", q.weight = "Range of the weight function",bw.selec = "bw.selec", mise = "Bootstrap MISE", trim = "Trimming percentage of MISE", grid.pil = "Grid used for the pilot bandwidth", pilot.bw = "Pilot bandwidth(s)", bandwidth = "Bandwidth(s)", grid.bw = "Grid used for bandwidth selection", p.hat = "Estimate of the conditional probability of uncensoring", x.est = "x.est", estimand = "Estimand")
      inx <- more %in% names(x)
      if(any(inx)){
